...
> **500명이 동시에 300개 한정 쿠폰에 몰릴 때, 정확히 300건만 발급되는가?**
...
원인: H2 데이터베이스는 행(row) 수준 락이 아닌 테이블(table) 수준 락을 사용
     → 500개 트랜잭션이 순차 직렬화되어 Connection Timeout 속출
```
...
```
쿠폰 재고: 300개
요청 인원: 500명 (스레드 500개 동시 실행)

회원 구성:
  - 475명: 포인트 100,000 (충분)
  -  25명: 포인트   9,000 (쿠폰 가격 10,000 → 부족)
           member 10, 30, 50, ... 490 (매 20번째 회원)

예상 결과:
  성공: 300건 (재고 한도)
  실패: 200건 (재고 소진 + 포인트 부족)
```
...
void concurrentCouponIssue() throws InterruptedException {
    ...
    assertThat(successCount.get()).isEqualTo(300);  // 정확히 300건
    assertThat(failCount.get()).isEqualTo(200);
}
```
